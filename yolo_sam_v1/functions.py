...
from constants import (yolo12_model_config, yolo12_model_path, yolo12_model_name,
                       ...
                       results_root, results_fig_root, results_inf_root, results_inf_all_root,
...
class InferenceSaver(ModelLoader):
    ...
    def save_all_inferences(self):
        ...
        for idx, img_file in enumerate(image_files, 1):
            ...
            if len(boxes_xyxy) > 0:
                ...
                self._save_inference_ints(bbox, output_path)
    ...
    def inference(self, image_path):
        """
        Run inference on a single image and save prediction as a YOLO-format .txt file.
        """
        ...
        if len(boxes_xyxy) > 0:
            ...
            # Output file name matches image name
    ...
    def _save_inference_ints(self, bbox, output_path):
        """
        Save one bounding box in XYXY format:
        x_min y_min x_max y_max

        Args:
            bbox: numpy array [x1, y1, x2, y2] or None
            output_path: Path to save the .txt file
        """
        with open(output_path, "w") as f:
            if bbox is None:
