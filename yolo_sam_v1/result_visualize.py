...
def visualize_result(
    ...
    # --------------------------------------------------
    # Convert binary mask:
    # foreground/polyp = 255 (white)
    # background       = 0   (black)
    sam_mask_image = (
        (pred_mask > 0).astype(np.uint8) * 255
    )
    ...
    axes[1].set_title("YOLO Bounding Box", fontsize=20)

    axes[2].imshow(gt_bbox_image)
    axes[2].set_title("Ground Truth Bounding Box", fontsize=20)
    ...
    axes[3].set_title("SAM Predicted Mask", fontsize=20)
    ...
    axes[4].set_title("Ground Truth Mask", fontsize=20)
